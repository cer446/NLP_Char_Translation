...
def main():
    ...
    # Define/Load models
    if os.path.exists(conf.save_path+"/encoderChar") and not conf.debug_mode:
        encoder = torch.load(conf.save_path+"/encoderChar")
        decoder = torch.load(conf.save_path+"/decoderChar")
    else:
        encoder = Encoder(
                conf.source_emb, 
                conf.hid_dim, 
    ...
    print([len(s) for s in train_source_seqs])
    ...
    for epoch in range(conf.epochs):
        ...
        if bleu > best_bleu:
            ...
            torch.save(encoder, conf.save_path+"/encoderChar")
            torch.save(decoder, conf.save_path+"/decoderChar")
